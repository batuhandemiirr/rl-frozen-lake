...
if secili_mod == "Klasik Q-Learning":
    ...
    with col_ayarlar:
        ...
        gamma = st.slider("İndirim Faktörü (Gamma)", 0.1, 1.0, 0.95, help="Gelecekteki ödüllerin önemi. Slippery Mode açıkken bu değeri yüksek tutmak ajanın uzağı görmesini sağlar.")
    ...
    if st.button("🚀 Q-Learning Eğitimini Başlat"):
        ...
        with col_tablo:
            ...
            df_q = pd.DataFrame(q_table, columns=actions_str)
...
# =====================================================================
    ...
    st.write("Ajan, Hareketli Hedef (Moving Target) problemini çözmek için **Policy Network (Ana Ağ)** ve **Target Network (Hedef Ağ)** kullanır.")
    
    col_ayarlar, _ = st.columns([1, 2])
    with col_ayarlar:
        episodes_dqn = st.number_input("Bölüm Sayısı", 100, 3000, 1000, 100)
        lr = st.slider("Öğrenme Oranı (Learning Rate)", 0.0001, 0.01, 0.001, 0.0001)
        gamma_dqn = st.slider("İndirim Faktörü (Gamma)", 0.1, 1.0, 0.95)
        target_update_freq = st.slider("Hedef Ağ Güncelleme Sıklığı", 1, 50, 10)
    ...
    if st.button("🚀 Çift Ağlı DQN Eğitimi Başlat"):
        ...
        status_text = st.empty()
        status_text.info("DQN Eğitimi başlıyor. Target Network sayesinde stabil öğrenme gerçekleşecek...")
        progress_bar = st.progress(0)
        ...
        for episode in range(episodes_dqn):
            ...
            while not (term or trunc):
                ...
                if len(memory) >= batch_size:
                    ...
                    b_actions = torch.tensor([s[1] for s in batch], dtype=torch.int64).unsqueeze(1)
                    ...
                    with torch.no_grad():
                        ...
                        target_q = b_rewards + (gamma_dqn * max_next_q * (1 - b_dones))
            ...
            epsilon = max(min_epsilon, epsilon * epsilon_decay)
            if episode % target_update_freq == 0: target_net.load_state_dict(policy_net.state_dict())
        ...
        st.markdown("---")
        st.subheader("🎬 1. Ajanın Hedefe Gidişi (Canlı İzleme)")
        col_img1, col_img2, col_img3 = st.columns([1, 2, 1])
        with col_img2: image_placeholder = st.empty()
            
        col1, col2 = st.columns(2)
        with col1:
            st.subheader("📉 2. Eğitim Kaybı (Loss)")
            if loss_history:
        ...
        with col2:
            st.subheader("📊 3. Sanal Q-Tablosu (Ana Ağ)")
            q_table_approx = np.zeros((state_size, action_size))
            with torch.no_grad():
                for s in range(state_size):
                    q_table_approx[s] = policy_net(state_to_tensor(s, state_size)).numpy()
            df_q = pd.DataFrame(q_table_approx, columns=actions_str)
            st.dataframe(df_q.style.background_gradient(cmap='RdYlGn', axis=1))

        st.subheader("📈 4. Bölüm Başına Ödül (Hareketli Ortalama)")
        if reward_history:
            smoothed = pd.Series(reward_history).rolling(window=50, min_periods=1).mean()
            fig2, ax2 = plt.subplots(figsize=(10, 3))
            ax2.plot(smoothed, color='green')
            st.pyplot(fig2)
        ...
        while True:
            ...
            while not (term or trunc):
                with torch.no_grad():
                    action = torch.argmax(policy_net(state_to_tensor(state, state_size))).item()
                state, r, term, trunc, _ = env.step(action)
                frame = env.render()
...
# =====================================================================
    ...
    with col_info:
        ...
        st.metric(label="Geçen Toplam Adım", value=st.session_state.m_step_count)        for episode in range(episodes):
            state, _ = env.reset()
            terminated = truncated = False
